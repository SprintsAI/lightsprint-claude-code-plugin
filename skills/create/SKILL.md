---
name: create
description: Create a new task in the active (or workspace default) stack. Use to add work items directly from Claude Code.
---

Run this command to create a new Lightsprint task. It lands in the active stack chosen via `lightsprint stacks use`, or the workspace default stack if none is chosen:
...
| `--stack <ref>` | No | Create the task in a specific stack. Accepts a stack ID, task prefix, or name. Use `lightsprint stacks` to list stacks. Defaults to the active stack (`lightsprint stacks use`), else the workspace default stack. |
